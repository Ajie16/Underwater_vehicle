...
{
		GY_IIC_Start();
		GY_IIC_Send_Byte(MS5837_30BA_WriteCommand);//发送写命令
	  GY_IIC_Wait_Ack();
    GY_IIC_Send_Byte(MS5837_30BA_ResetCommand);//发送复位命令
	  GY_IIC_Wait_Ack();
    GY_IIC_Stop();
...
u8 MS5837_30BA_PROM(void)
 {	 
  ...
  delay_ms(20); 
    ...
    GY_IIC_Send_Byte(MS5837_30BA_WriteCommand);      //发送写命令
		GY_IIC_Wait_Ack();
		GY_IIC_Send_Byte(MS5837_30BA_PROM_RD + (i*2));   //发送PROM获取命令
		GY_IIC_Wait_Ack();
    GY_IIC_Stop();
		delay_us(5);
		GY_IIC_Start();
		GY_IIC_Send_Byte(MS5837_30BA_ReadCommand);       //进入接收模式
		delay_us(5);
		GY_IIC_Wait_Ack();
		inth = GY_IIC_Read_Byte(1);  		                //带ACK的读数据
		delay_us(5);
		intl = GY_IIC_Read_Byte(0); 			                //最后一个字节NACK
		GY_IIC_Stop();
...
unsigned long MS5837_30BA_GetConversion(u8 command)
{
			...
			u8 temp[3];
	    GY_IIC_Start();
			GY_IIC_Send_Byte(MS5837_30BA_WriteCommand); 		//写地址
			GY_IIC_Wait_Ack();
			GY_IIC_Send_Byte(command); //写转换命令
			GY_IIC_Wait_Ack();
			GY_IIC_Stop();
			...
			GY_IIC_Send_Byte(MS5837_30BA_ReadCommand);  //进入接收模式
			GY_IIC_Wait_Ack();
			temp[0] = GY_IIC_Read_Byte(1);  //带ACK的读数据  bit 23-16
			temp[1] = GY_IIC_Read_Byte(1);  //带ACK的读数据  bit 8-15
			temp[2] = GY_IIC_Read_Byte(0);  //带NACK的读数据 bit 0-7
			GY_IIC_Stop();
